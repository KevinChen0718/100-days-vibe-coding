...
你**不能直接控制他**——只能**事先在封閉的房子裡擺好場景物件**鋪出一條安全的路，
再「放他走」。**放走之後道具就鎖定、不能再動**，只能看他走完。
別讓他摔太重而驚醒、別讓他踩到尖刺或掉進水裡，把他平安送下樓、走到出口。

關卡是封閉的房子剖面（牆、天花板、樓層、管道把空間關住），配上即時生成的慵懶 lounge jazz，
就是一片睡意朦朧的夜。
...
- **擺道具**：滑鼠或手指拖動場景中的道具到定位。**只能在放他走之前擺**。
- **放他走 ▶**：先生開始夢遊，道具就鎖定了，接下來只能看他走完。
- **重來**：把先生放回床上、重新佈置道具。
- **提示**：顯示物件該擺放位置的虛影。
- 鍵盤：`空白鍵` 放他走 / 下一關，`R` 重來，`H` 提示。

> 設計重點：道具放走後鎖定，所以你不能「拖一塊板子當移動地板把他推到終點」——每一關都得想清楚正確的擺法。而且每關發的道具配額剛好，**每個都非用不可、一個都不能省**。
...
四個章節、共 **9 關**，是一棟房子的剖面，引導夢遊先生從頂樓臥室一路安全下到地面大門。難度循序漸進、後段組合多道具與傳送門：
