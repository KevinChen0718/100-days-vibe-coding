...
(function (root) {
  ...
  function drawSolids(ctx, solids, W, H) {
    ...
    walls.forEach(function (s) {
      ...
      if (s.h > 280) {                  // 邊界高樓
        ...
        ctx.fillStyle = '#566089'; roundRect(ctx, s.x, s.y, s.w, s.h, 3); ctx.fill(); outline(ctx, 2.5);
        ctx.fillStyle = '#3a4570'; ctx.fillRect(s.x - 3, s.y, s.w + 6, 8);
        ctx.strokeStyle = OUT; ctx.lineWidth = 2; ctx.strokeRect(s.x - 3, s.y, s.w + 6, 8);
      }
    });
  }
  ...
  function drawMovable(ctx, m, opts) {
    ...
    if (m.type === 'plank') {
      ...
      pg.addColorStop(0, '#d2a063'); pg.addColorStop(1, '#a06f3c');
      ...
      for (var gx = m.x + 20; gx < m.x + m.w; gx += 28) { ctx.beginPath(); ctx.moveTo(gx, m.y + 2); ctx.lineTo(gx, m.y + m.h - 2); ctx.stroke(); }
    } else if (m.type === 'box') {
      ...
      for (var sc = 0; sc * 18 < m.w; sc++) { ctx.beginPath(); ctx.arc(m.x + 9 + sc * 18, m.y + m.h, 9, 0, Math.PI); ctx.fill(); }
    }
    ctx.restore();
  ...
  function drawPortal(ctx, p, time) {
    ...
    g.addColorStop(0, hexA(col, 0.45 + 0.2 * Math.sin(time * 0.005))); g.addColorStop(1, hexA(col, 0));
    ctx.fillStyle = g; ctx.fillRect(cx - p.w * 1.8, cy - p.w * 1.8, p.w * 3.6, p.w * 3.6);
    ctx.fillStyle = 'rgba(18,16,34,0.92)'; roundRect(ctx, p.x - 4, p.y - 4, p.w + 8, p.h + 8, 13); ctx.fill(); outline(ctx, 2.5);
    ctx.save();
    ctx.beginPath(); ctx.ellipse(cx, cy, rx, ry, 0, 0, 7); ctx.clip();
    var sg = ctx.createLinearGradient(cx, p.y, cx, p.y + p.h);
    sg.addColorStop(0, hexA(col, 0.85)); sg.addColorStop(0.5, 'rgba(10,8,24,0.95)'); sg.addColorStop(1, hexA(col, 0.85));
    ctx.fillStyle = sg; ctx.fillRect(p.x, p.y, p.w, p.h);
    ctx.strokeStyle = hexA(col, 0.6); ctx.lineWidth = 2;
    for (var i = 1; i <= 3; i++) { var rr = ry * (i / 3.5); ctx.beginPath(); ctx.ellipse(cx, cy, rr * 0.7, rr, time * 0.02 * (i % 2 ? 1 : -1), 0, 5); ctx.stroke(); }
    ctx.restore();
    ctx.strokeStyle = hexA(col, 0.95); ctx.lineWidth = 2.5; ctx.beginPath(); ctx.ellipse(cx, cy, rx, ry, 0, 0, 7); ctx.stroke();
    for (var k = 0; k < 3; k++) { var ang = time * 0.04 + k * 2.094; ctx.fillStyle = hexA(col, 0.9); ctx.beginPath(); ctx.arc(cx + Math.cos(ang) * (rx + 4), cy + Math.sin(ang) * (ry + 4), 2, 0, 7); ctx.fill(); }
  }
  ...
  root.SleepwalkerSprites = {
    ...
    drawPortal: drawPortal, drawMover: drawMover, drawWalker: drawWalker, roundRect: roundRect
  };
