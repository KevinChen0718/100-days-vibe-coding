...
- **Day 3（現在）**：`queryRoute()` 改成**兩段式真實抓取**——直接 `fetch` Alaska（不開瀏覽器、不用登入），把「便宜候選日」和「可通知的真航班」分開。
  - **Stage 1 月曆雷達**：`POST /search/api/shoulderDates`（`fareView:"as_awards"`）一次回錨點 ±15 天（≈31 天）的每日最低獎勵票里程。這個數字不分艙等／航空公司，只能當候選日雷達。
  - **Stage 2 逐日確認**：只對月曆達標候選日打 `GET /search/results/__data.json`，確認追蹤航司（預設 `JX`，route 可設 `carriers`）真的有 ≤ 門檻的航班，並寫入航司／航班號／起降時間／里程／剩位／確認時間。
  - 逐日端點有 token-bucket 限速，單次掃描全部航線合計最多查 5 天、每次間隔至少 2.5 秒；額度用完的候選日會留到下輪。
  - 通知只對 Stage 2 確認的真航班發送；月曆達標但未確認會在儀表板淡色標「未確認（混航司最低價）」。
  - `RADAR_SOURCE=mock npm run scan` 可強制走離線假資料（開發 / demo），會產生模擬航班列，但絕不真的推播。
